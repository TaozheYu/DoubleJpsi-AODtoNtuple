void Calculate_corrector_v2(const char *mc, const char *type){
  //gROOT->Reset();
  ...
  char openFileGen[500];  sprintf(openFileGen, "Gen_Event_%s_private.root",mc);
  char openFileReco[500];  sprintf(openFileReco, "Gen_Event_%s_%s.root",mc,type);
  TFile *fileJpsi_gen = new TFile(openFileGen);
  TFile *fileJpsi_reco = new TFile(openFileReco);
  ...
  TH2F *h_acc_pt_Jpsi_official = (TH2F*) fileJpsi_reco->Get("h_acc_pt_Jpsi");
  TH2F *h_reco_Jpsi = (TH2F*) fileJpsi_reco->Get("h_reco_Jpsi");
  TH2F *h_eff_Jpsi = (TH2F*) fileJpsi_reco->Get("h_eff_Jpsi");
  TH2F *h_vtx_Jpsi = (TH2F*) fileJpsi_reco->Get("h_vtx_Jpsi");
  
  TH2F *h_JpsiPair = (TH2F*) fileJpsi_reco->Get("h_JpsiPair");
  TH2F *h_HLT_JpsiPair = (TH2F*) fileJpsi_reco->Get("h_HLT_JpsiPair");
  TH2F *h_evt_JpsiPair = (TH2F*) fileJpsi_reco->Get("h_evt_JpsiPair");
  ...
  for(int i=1; i<h_total_Jpsi->GetXaxis()->GetNbins()+1; i++){
    ...
    float N_acc_pt_official  = h_acc_pt_Jpsi_official  ->GetBinContent(i,j);
    ...
    if (N_total!=0&&N_acc_pt!=0&&N_reco!=0&&N_eff!=0&&N_vtx!=0){ 
      ...
      float w_reco_err= Cal_stat_err(N_reco,N_acc_pt_official);
