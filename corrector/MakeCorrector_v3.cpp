...
void MakeCorrector_v3(const char *mc, const char *type){
  ...
  TH2F *h_total_Jpsi1 = new TH2F("h_total_Jpsi1","h_total_Jpsi1",N-1 , xbins, M-1, ybins);
  TH2F *h_total_Jpsi2 = new TH2F("h_total_Jpsi2","h_total_Jpsi2",N-1 , xbins, M-1, ybins);
  TH2F *h_acc_eta_Jpsi1 = new TH2F("h_acc_eta_Jpsi1","h_acc_eta_Jpsi1",N-1 , xbins, M-1, ybins);
  TH2F *h_acc_eta_Jpsi2 = new TH2F("h_acc_eta_Jpsi2","h_acc_eta_Jpsi2",N-1 , xbins, M-1, ybins);
  TH2F *h_acc_pt_Jpsi1 = new TH2F("h_acc_pt_Jpsi1","h_acc_pt_Jpsi1",N-1 , xbins, M-1, ybins);
  TH2F *h_acc_pt_Jpsi2 = new TH2F("h_acc_pt_Jpsi2","h_acc_pt_Jpsi2",N-1 , xbins, M-1, ybins);
  TH2F *h_reco_Jpsi1 = new TH2F("h_reco_Jpsi1","h_reco_Jpsi1",N-1 , xbins, M-1, ybins);
  TH2F *h_reco_Jpsi2 = new TH2F("h_reco_Jpsi2","h_reco_Jpsi2",N-1 , xbins, M-1, ybins);
  TH2F *h_eff_Jpsi1 = new TH2F("h_eff_Jpsi1","h_eff_Jpsi1",N-1 , xbins, M-1, ybins);
  TH2F *h_eff_Jpsi2 = new TH2F("h_eff_Jpsi2","h_eff_Jpsi2",N-1 , xbins, M-1, ybins);
  TH2F *h_vtx_Jpsi1 = new TH2F("h_vtx_Jpsi1","h_vtx_Jpsi1",N-1 , xbins, M-1, ybins);
  TH2F *h_vtx_Jpsi2 = new TH2F("h_vtx_Jpsi2","h_vtx_Jpsi2",N-1 , xbins, M-1, ybins);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(!((fabs(GENups_Daughter_mueta->at(0))<2.4)&&(fabs(GENups_Daughter_mueta->at(1)&&(fabs(GENups_Daughter_mueta->at(2))<2.4))<2.4)&&(fabs(GENups_Daughter_mueta->at(3))<2.4))) continue; 
    ...
    */ 
    ...
    h_total_Jpsi1->Fill(GENdimu_pt->at(0),GENdimu_y->at(0));
    h_total_Jpsi2->Fill(GENdimu_pt->at(1),GENdimu_y->at(1));


    if (!(abs(GENmu_eta->at(GEN_ups1_mu1_index))<2.4 && abs(GENmu_eta->at(GEN_ups1_mu2_index))<2.4 && abs(GENmu_eta->at(GEN_ups2_mu1_index))<2.4 && abs(GENmu_eta->at(GEN_ups2_mu2_index))<2.4)) continue;
    h_acc_eta_Jpsi1->Fill(GENdimu_pt->at(0),GENdimu_y->at(0));    
    h_acc_eta_Jpsi2->Fill(GENdimu_pt->at(1),GENdimu_y->at(1));    

    if (!(abs(GENmu_pt->at(GEN_ups1_mu1_index))>3.5 && abs(GENmu_pt->at(GEN_ups1_mu2_index))>3.5 && abs(GENmu_pt->at(GEN_ups2_mu1_index))>3.5 && abs(GENmu_pt->at(GEN_ups2_mu2_index))>3.5)) continue;
    h_acc_pt_Jpsi1->Fill(GENdimu_pt->at(0),GENdimu_y->at(0));    
    h_acc_pt_Jpsi2->Fill(GENdimu_pt->at(1),GENdimu_y->at(1));    
    ...
    h_reco_Jpsi1->Fill(ups1_pt_GenMatched,ups1_y_GenMatched);
    h_reco_Jpsi2->Fill(ups2_pt_GenMatched,ups2_y_GenMatched);
    
    if (!((ups1_pt_GenMatched_ID>0)&&(ups2_pt_GenMatched_ID>0))) continue;  
    
    h_eff_Jpsi1->Fill(ups1_pt_GenMatched_ID,ups1_y_GenMatched_ID);
    h_eff_Jpsi2->Fill(ups2_pt_GenMatched_ID,ups2_y_GenMatched_ID);

    if (!((ups1_pt_GenMatched_ID_OS_VTX>0)&&(ups2_pt_GenMatched_ID_OS_VTX>0))) continue;  
    if (!(fourMuFit_ups1_VtxProb->size()>0 && fourMuFit_ups2_VtxProb->size()>0)) continue;
    if (!(fourMuFit_ups1_VtxProb->at(0)>0.005 && fourMuFit_ups2_VtxProb->at(0)>0.005)) continue;
     
    h_vtx_Jpsi1->Fill(ups1_pt_GenMatched_ID_OS_VTX,ups1_y_GenMatched_ID_OS_VTX);
    h_vtx_Jpsi2->Fill(ups2_pt_GenMatched_ID_OS_VTX,ups2_y_GenMatched_ID_OS_VTX);
  ...
  h_total_Jpsi1->Write();
  h_total_Jpsi2->Write();
  h_acc_eta_Jpsi1->Write();
  h_acc_eta_Jpsi2->Write();
  h_acc_pt_Jpsi1->Write();
  h_acc_pt_Jpsi2->Write();
  h_reco_Jpsi1->Write();
  h_reco_Jpsi2->Write();
  h_eff_Jpsi1->Write();
  h_eff_Jpsi2->Write();
